...
async function statusCommand(sock, from, msg, isAdmin, botData, saveBotData, userId, args) {
    ...
    if (!action) {
        ...
        const menu = `╭━━━〔 ${toBold("STATUS SETTINGS")} 〕━━━┈⊷\n` +
                   ...
                   `┃ ⋄ ${toBold("Auto Save Status:")} ${settings.autoSaveStatus ? '✅' : '❌'}\n` +
    ...
    if (action === 'autosavestatus') {
        ...
        if (value === 'all') {
            ...
            saveBotData();
            await sock.sendMessage(from, { text: '✅ *AUTO SAVE STATUS: ALL ON*\n\nReceived contact and group statuses will be forwarded to your personal inbox.' }, { quoted: msg });
        } else if (value === 'off') {
            botData.statusSettings[userId].autoSaveStatus = false;
            saveBotData();
            await sock.sendMessage(from, { text: '❌ *AUTO SAVE STATUS: OFF*' }, { quoted: msg });
        } else {
            await sock.sendMessage(from, { text: `Usage: ${settings.prefix}autosavestatus all/off` }, { quoted: msg });
        }
    } else if (action === 'on') {
        botData.statusSettings[userId].autoStatus = true;
