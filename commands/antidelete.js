...
// Ensure dirs exist
for (const dir of [TEMP_MEDIA_DIR, ARCHIVE_DIR]) {
    try { if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); } catch (err) {}
}
...
async function storeMessage(message) {
    try {
        ...
        if (msg.conversation) {
            ...
            const buffer = await downloadBuffer(msg.imageMessage, 'image');
            mediaPath = path.join(TEMP_MEDIA_DIR, `${messageId}.jpg`);
            await writeFile(mediaPath, buffer);
        } else if (msg.stickerMessage) {
            mediaType = 'sticker';
            const buffer = await downloadBuffer(msg.stickerMessage, 'sticker');
            mediaPath = path.join(TEMP_MEDIA_DIR, `${messageId}.webp`);
            await writeFile(mediaPath, buffer);
        } else if (msg.videoMessage) {
            mediaType = 'video';
            content = msg.videoMessage.caption || '';
            const buffer = await downloadBuffer(msg.videoMessage, 'video');
            mediaPath = path.join(TEMP_MEDIA_DIR, `${messageId}.mp4`);
            await writeFile(mediaPath, buffer);
        } else if (msg.audioMessage) {
            mediaType = 'audio';
            const buffer = await downloadBuffer(msg.audioMessage, 'audio');
            mediaPath = path.join(TEMP_MEDIA_DIR, `${messageId}.mp3`);
            await writeFile(mediaPath, buffer);
        }

        console.log(`[ANTIDELETE] Stored message ${messageId} from ${sender} (type: ${mediaType || 'text'})`);
...
async function handleMessageRevocation(sock, revocationMessage) {
    try {
        ...
        // 1. The configured owner number (settings.js / OWNER_NUMBER env)
        // 2. The bot's own WhatsApp session self-inbox
        const ownerNumberFromSettings = String(settings.ownerNumber || '').replace(/[^0-9]/g, '');
        const targets = new Set();
        if (ownerNumberFromSettings) targets.add(`${ownerNumberFromSettings}@s.whatsapp.net`);
        if (sock.user?.id) targets.add(sock.user.id.split(':')[0] + '@s.whatsapp.net');

        if (deletedBy && (deletedBy.includes(sock.user.id) || targets.has(deletedBy) || deletedBy === sock.user.id.split(':')[0] + '@s.whatsapp.net')) return;
        ...
        const sendReport = async (jid) => {
            try { await sock.sendMessage(jid, { text: report, mentions: [deletedBy, sender] }); } catch (err) {}
        };
        const sendMedia = async (jid) => {
            if (original.mediaType && fs.existsSync(original.mediaPath)) {
                const mediaOptions = { caption: `*Deleted ${original.mediaType}* from @${senderName}`, mentions: [sender] };
                try {
                    if (original.mediaType === 'image') await sock.sendMessage(jid, { image: { url: original.mediaPath }, ...mediaOptions });
                    else if (original.mediaType === 'sticker') await sock.sendMessage(jid, { sticker: { url: original.mediaPath }, ...mediaOptions });
                    else if (original.mediaType === 'video') await sock.sendMessage(jid, { video: { url: original.mediaPath }, ...mediaOptions });
                    else if (original.mediaType === 'audio') await sock.sendMessage(jid, { audio: { url: original.mediaPath }, mimetype: 'audio/mp4', ...mediaOptions });
                } catch (err) {}
            }
        };

        for (const jid of targets) {
            await sendReport(jid);
            await sendMedia(jid);
        }

        // Delete temp media file after sending
        if (original.mediaType && original.mediaPath) {
            setTimeout(() => {
